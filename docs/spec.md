...
一个**工具类**微信小程序：把 B站视频存成「卡片」（标题 / 封面 / UP主 / 分区），用**标签**归类，按**收藏月份**分组，点击跳回 B站查看。**只存元数据 + 缩略图，不播视频、不存视频本体。**
...
- ❌ 多平台（抖音/小红书/X/油管）—— 未来
- ❌ 视频下载 / 一键发布 —— 未来（复用 hermes 基建，先想清版权）
- ❌ 打赏 / 订阅（个人主体无虚拟支付）
- ❌ 视频搜索（未来）
...
- **幂等 & 控频（防刷码）**：同一 `user.mid` 只发一次激活码（服务端记录已发 UID）；发送控频（每账号间隔数秒）；激活码一次性、绑定后即失效，防「取关→再关注」刷码
