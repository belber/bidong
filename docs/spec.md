...
- **@ 触发时才回私信（2026-09-27 起，取代原来的"默认不回复"）**：
  只在用户主动 @ 我们时回，且只走私信、不进评论区（评论区回复更容易被判刷屏/引流）。
  原来的"完全不回"会让用户不确定成功没成功，于是反复 @ 试探——重复 @ 反而制造更多互动量，
  所以改成带频控的回复。总开关 `enable_at_reply`（管理端「功能开关」），风控一响一键关。
  | 情况 | 回什么 |
  |:--|:--|
  | 没关注就 @ | 引导关注（**不发码**，关注才是给码的回报） |
  | 已关注但没绑定 | 重发同一个激活码（多半是私信漏看） |
  | 已绑定、收藏成功 | 收藏确认，带视频标题 + 小程序名 |
  | 已绑定、收藏失败 | 不回，只记 `解析失败` |
  | 24h 内已回过（未绑定） | 不回，记 `unbound_quiet` |
  | 24h 内确认已回满 3 条（已绑定） | 不回，记 `collected_quiet` |
  - 判据：`binding` 行存在 = 关注过（关注时就会建行）；没有 = 没关注过。
  - 频控：未绑定同一 UID 24h 只回一次；已绑定同一 UID 24h 最多回 3 条。
  - 一条 @ 只落一行 `at_event`：结果记成 `replied_*` / `collected_quiet` / `reply_failed`，管理端「评论@监控」直接看得到。
