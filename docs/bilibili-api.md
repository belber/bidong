...
先说明一个关键认知：**B站没有「机器人账号」这种官方类型**。我们说的「@壁咚收藏 机器人」，本质就是一个**普通 B站账号**，靠后台持有它的登录态 Cookie 去「收私信、解析 @ 指令、发评论回复」。所以「查机器人账号」就是「查一个普通账号」，用下面同一套接口。

### 1.1 按昵称搜索用户（查重 / 查账号是否存在）✅

```bash
curl "https://api.bilibili.com/x/web-interface/search/type?search_type=bili_user&keyword=壁咚收藏"
```

- 用途：起名时查这个昵称是否已被人占用（就是之前用来验证「壁咚收藏」是否可用、以及看别人机器人怎么命名的接口）。
